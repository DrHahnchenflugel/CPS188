...
int main(void){
	...
	*/
	...
	//OTHER ISSUES:
	- Old iteration of program could be running, taking up same memloc, 
		causing unpredictable variable values
	- Cannot assign values when initializing variables if assignment involves 
		operations on other variables (+,-,*,/,etc).
	*/
	...
	*/
	...
	//SCANF
	int miles;					//THE & SIGN IS VERY IMPORTANT
	printf("input number of miles:");
	scanf("%d", &miles); //User input
	printf("miles is:%d\n", miles);
	...
	//3] Write to file
	fprintf(my_output_file, "%lf", miles_times_pi);
	//4] Close file
